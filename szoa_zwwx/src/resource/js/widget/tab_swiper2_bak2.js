...
define(['jquery'], function ($) {
    ...
    function renderSwiper(swiperconfig) {
        ...
        tabSlide(tabMap.index, tabMap.callback);
    }
        ...
        if (index !== null) {
            ...
            */
            if (fn && typeof fn === 'function') {
                fn.call(null, tempindex, tabEffect);
            } else {
                if (swiper_instance !== null) {
                    if (swiper_id !== null) {
    ...
    /*tab切换效果*/
    function tabEffect(tempindex) {
        tabMap.index = tempindex;
        tabMap.$tab_item.eq(tempindex).addClass('tabactive active').siblings().removeClass('tabactive active');
        tabMap.$grid_item.eq(tempindex).addClass('tabactive active').siblings().removeClass('tabactive active');
        position(tempindex);
        if (isColumn()) {
            toggle();
        }
    ...
    function bind() {
        ...
        $.each([tabMap.$tab_wrap, tabMap.$tabswiper_grid], function () {
            ...
            this.on(event_adapt.click, function (e) {
                ...
                tabSlide(tempindex, tabMap.callback);
            });
        });
        /*绑定显示隐藏菜单模式*/
    ...
    /*修正swiper位置*/
    function correctSwiper(index) {
        index = typeof index !== 'undefined' ? index : tabMap.index;
