...
define(['jquery'], function ($) {
    ...
    var tabMap = {
        ...
        item_width: this.win_width/*每个菜单的宽度*/,
        total_width: this.win_width/*默认总宽度*/,
        parent_class: ''/*父级样式*/,
        item_class: ''/*平均分配样式，支持的菜单个数*/,
        slide_size: 0/*slide数量*/,
        count: 1/*菜单多少页*/,
        cssleft: 0/*当前菜单位置*/,
        swiper_instance: null/*实例化swiper对象*/
    };
    ...
    function bind(swiperconfig) {
        ...
        if (tabMap.isbodyhide) {
            tabMap.$body.on('click', function (e) {
                ...
                if (isColumn()) {
                    tabMap.$tab_btn.trigger('click');
                }
            });
        }

        /*绑定swipe事件*/
        if (tabMap.swiper_instance === null) {
            /*没有绑定则绑定*/
            var tempconfig = {
                speed: 500,
                onSlideChangeStart: function () {
                    var tempindex = tabMap.swiper_instance.activeIndex;
            ...
            /*创建swiper实例*/
            tabMap.swiper_instance = new Swiper('.swiper-container', tempconfig);
            /*绑定点击切换事件*/
            tabMap.$tab_wrap.on('touchstart mousedown', function (e) {
                e.stopPropagation();
                ...
                position(tempindex);
                $this.addClass('tabactive active').siblings().removeClass('tabactive active');
                tabMap.swiper_instance.swipeTo(tempindex);
            });
        }

        /*绑定切换屏重现渲染*/
        /*var _resize=debouce(resize(),200);
    ...
    * */
    ...
    return {
        ...
        swiper_instance: tabMap.swiper_instance/*swiper实例对象*/
    };
});