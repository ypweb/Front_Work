    ...
    // function InitButton() {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (btnList[i].name == '送出') {
                btn_str += '<li class="tool-menu-send menu-extend-send" data-type="' + btnList[i].id + '" data-title="' + btnList[i].name + '"></li>'
            } else if (btnList[i].name == '保存') {
                btn_str += '<li class="tool-menu-save" data-type="' + btnList[i].id + '" data-title="' + btnList[i].name + '"></li>'
            } else if (btnList[i].name == '退件') {
                btn_str += '<li class="tool-menu-return" data-type="' + btnList[i].id + '" data-title="' + btnList[i].name + '"></li>'
            } else if (btnList[i].name == '送经办人') {
                btn_str += '<li class="tool-menu-handle" data-type="SendSpecial" data-title="' + btnList[i].name + '"></li>'
            } else if (btnList[i].name == '办结') {
                btn_str += '<li class="tool-menu-finish" data-type="SendEnd" data-title="' + btnList[i].name + '"></li>'
            } else if (btnList[i].name == '退文') {
                if ((NodeID == DOCUMENT_NODE5 || NodeID == DOCUMENT_NODE9) && btnList[i].id == SYSTEM_REJECT) {
                    continue;
                } else { //退文
                    btn_str += '<li class="tool-menu-back" data-type="SendFileBack" data-title="' + btnList[i].name + '"></li>'
                }
            }
        }
    ...
    function forSure(flowMsg,nodeid,userId) {
        if(flowMsg.length>0){
            for (var i = 0; i < flowMsg.length; i++) {
                ...
                var authorid = msgInfo.authorid.split(";");
