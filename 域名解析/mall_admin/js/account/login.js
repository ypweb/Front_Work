...
(function($){
	$(function(){
		...
		if(public_tool.initMap.isrender){
				...
				submitHandler: function(form){
					...
					$.ajax({
						...
						show_loading_bar({
							...
							finish: function(){
								if(code===0){
									location.href=location.href.replace('/mall_admin','/operator_admin').replace('/app/account/login.html','/index.html');
								}
							}
						});
			...
			$loginform_mall.validate({
				...
				submitHandler: function(form){
					...
					var basedomain='http://112.74.207.132:8081',
		...
		function getValidCode(type){
			...
			if(type==='provider'){
				/*供应商入口*/
				xhr.open("post",'http://112.74.207.132:8082/yttx-providerbms-api/user/identifying/code', true);
			}else if(type==='mall'){
				/*布住网*/
				xhr.open("post",'http://112.74.207.132:8081/mall-buzhubms-api/sysuser/identifying/code', true);
			}else{
				/*其他*/
				xhr.open("post",'http://112.74.207.132:8081/mall-buzhubms-api/sysuser/identifying/code', true);
			}
