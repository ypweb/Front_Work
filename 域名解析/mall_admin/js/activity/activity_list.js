	...
	$(function(){
		...
		if(public_tool.initMap.isrender){
			...
			var powermap=public_tool.getPower(),
				manage_power=public_tool.getKeyPower('bzw-activity-list',powermap);
			...
			/*商品查询对象*/
			var $search_forwardType=$('#search_forwardType'),
			...
			var activity_page={
				...
				activity_config={
					...
					config:{
						...
						paging:false,
						ajax:{
							url:"http://112.74.207.132:8081/mall-buzhubms-api/placing/location/list",
							dataType:'JSON',
							method:'post',
			...
			$admin_search_clear.on('click',function(){
				$.each([$search_forwardType,$search_status],function(){
					this.val('');
				});
			}).trigger('click');
			...
			$admin_search_btn.on('click',function(){
				var data= $.extend(true,{},activity_config.config.ajax.data);
				...
				activity_config.config.ajax.data= $.extend(true,{},data);
				getColumnData(activity_page,activity_config);
			});
			...
			$admin_list_wrap.delegate('span','click',function(e){
				...
				if(action==='edit'){
					...
					public_tool.setParams('bzw-activity-add',tempdata);
					location.href='bzw-activity-add.html';
		...
		function setEnabled(obj){
			...
			$.ajax({
					url:"http://112.74.207.132:8081/mall-buzhubms-api/placing/location/updown",
					dataType:'JSON',
					method:'post',
					data:{
						id:id,
