...
angular.module('app')
    .service('dataTableColumnService', function () {
        ...
        this.initWidget = function (tablecolumn, tablecache) {
            ...
            if (tablecolumn.hide_header && tablecolumn.hide_header.length !== 0) {
                str = _calColumnHeader(tablecolumn, tablecache, true);
            } else {
                str = _calColumnHeader(tablecolumn, tablecache, false);
            }
        ...
        /*私有服务*/
        function _calColumnHeader(tablecolumn, tablecache, flag) {
            var index,
                str = '',
                i = 0;

            if (flag) {
                var header = tablecolumn.hide_header;
            }

            for (i; i < tablecolumn.hide_len; i++) {
                index = tablecolumn.hide_list[i];
                if (flag) {
                    str += '<li data-value="' + index + '">第' + (index + 1) + '列<em>(' + header[index] + ')</em></li>';
                } else {
                    str += '<li data-value="' + index + '">第' + (index + 1) + '列</li>';
                }
                tablecache.column(index).visible(false);
            }
            return str;
