    ...
    $(function () {
        ...
        if (public_tool.initMap.isrender) {
            ...
            var $storage_stats_wrap = $('#storage_stats_wrap')/*表格*/,
                ...
                resetform0 = null,
                goodsmap = {
                    goodsseqid: []/*所有checkbox值*/,
                    goodsactive: []/*选中的checkbox值*/,
                    goodsobj: []/*选中的checkbox*/
                },
                goodslistmap = {}/*查询出来的标签*/,
        ...
        function getGoodsList($code) {
            ...
            $.ajax({
                ...
                .done(function (resp) {
                    ...
                    if (list) {
                        var len = list.length,
                            sequenceid = $code.attr('data-seqid');
                        if (len !== 0) {
                            /*重置查询对象*/
                            createGoodsListMap(sequenceid, len);
                            /*解析数据*/
                            for (i; i < len; i++) {
                                ...
                                var attrlist = list[i]['list'],
                                    ...
                                    j = 0;
                                /*设置标签数据*/
                                setGoodsListMap(sequenceid, labelid);
                                for (j; j < attrlen; j++) {
                                    var attrname = attrlist[j]["name"],
                                        attrid = attrlist[j]["id"];

                                    str += '<span data-seqid="' + sequenceid + '" data-attrid="' + attrid + '" data-attrname="' + chartFilter(attrname, "on") + '" data-labelid="' + labelid + '"  data-labelname="' + chartFilter(labelname, "on") + '" >' + attrname + '</span>';
                                }
                                str += '</div>';
                            }
        ...
        /*操作查询对象*/
        function operateGoodsListMap(config) {
            if (config) {
                var seqid = config.seqid/*查询对象序列*/,
                    value = config.value/*查询对象值*/,
                ...
                if (typeof value !== 'undefined' && value !== '' && listmap['list'][value] && listmap.size !== 0) {
                    ...
                    if (type === 'add') {
                        /*新增*/
                        if (listmap.size > listmap.checked) {
                            /*小于操作数*/
                            listmap.checked++;
                        }
                    } else if (type === 'remove') {
                        /*删除*/
                        if (listmap.checked > 0) {
                            /*存在选中数据*/
                            listmap.checked--;
                        }
                    }
        ...
        /*创建查询对象*/
        function createGoodsListMap(seqid, size) {
            if (goodslistmap[seqid]) {
                /*存在即重置*/
                (typeof size !== 'undefined' && size !== 0) ? goodslistmap[seqid]['size'] = size : goodslistmap[seqid]['size'] = 0;
                goodslistmap[seqid]['list'] = {};
                goodslistmap[seqid]['checked'] = 0;
            } else {
                /*不存在即创建*/
                var tempmap = {};
                tempmap['size'] = (typeof size !== 'undefined' && size !== 0) ? size : 0;
                tempmap['list'] = {};
                tempmap['checked'] = 0;
                goodslistmap[seqid] = tempmap;
            }
        }

        /*删除查询对象*/
        function deleteGoodsListMap(seqid) {
            if (goodslistmap[seqid]) {
        ...
        function addStorageItem() {
            var seqid = (Math.random()).toString().slice(2, 15),
                str = '<tr>\
						...
						<td>\
							<input class="form-control goodscode" data-seqid="' + seqid + '" placeholder="请输入商品编码" data-value="" type="text" />\
						</td>\
						<td data-id=""></td>\
						<td></td>\
						<td>\
                        ...
                        if (len !== 0) {
                            for (i; i < len; i++) {
                                var tempstorage = list[i],
                                    gcode = typeof tempstorage["goodsCode"] === 'undefined' ? '' : tempstorage["goodsCode"];
