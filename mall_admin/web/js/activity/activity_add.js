...
(function ($) {
    ...
    $(function () {
        ...
        if (public_tool.initMap.isrender) {
            ...
            /*dom引用和相关变量定义*/
            var module_id = 'bzw-activity-add'/*模块id，主要用于本地存储传值*/,
                dia = dialog({
                    zIndex: 2000,
                    title: '温馨提示',
                    okValue: '确定',
                    ...
                    init: {
                        ...
                        'FileUploaded': function (up, file, info) {
                            ...
                            if (advertpos === 1) {
                                /*左上*/
                                $admin_urlImage.attr({
                                    'data-image': domain + '/' + name.key
                                }).html('<img src="' + domain + '/' + name.key + "?imageView2/1/w/340/h/480" + '" alt="图像">');
                            } else if (advertpos === 2) {
                                /*右上*/
                                $admin_urlImage.attr({
                                    'data-image': domain + '/' + name.key
                                }).html('<img src="' + domain + '/' + name.key + "?imageView2/1/w/644/h/240" + '" alt="图像">');
                            } else if (advertpos === 3 || advertpos === 4) {
                                /*右中，右下*/
                                $admin_urlImage.attr({
                                    'data-image': domain + '/' + name.key
                                }).html('<img src="' + domain + '/' + name.key + "?imageView2/1/w/306/h/208" + '" alt="图像">');
                            } else if (advertpos === 0) {
                                /*顶部*/
                                $admin_urlImage.attr({
                                    'data-image': domain + '/' + name.key
                                }).html('<img src="' + domain + '/' + name.key + "?imageView2/1/w/1080/h/500" + '" alt="图像">');
                            } else {
                                /*默认*/
                                $admin_urlImage.attr({
                                    'data-image': domain + '/' + name.key
                                }).html('<img src="' + domain + '/' + name.key + "?imageView2/1/w/160/h/160" + '" alt="图像">');
                            }
                        },
            ...
            /*获取编辑缓存*/
            (function () {
                var edit_cache = public_tool.getParams('bzw-activity-add');
                if (edit_cache) {
                    $admin_action.removeClass('g-d-hidei').html('编辑');
                    /*查询数据*/
                    setActivityData(edit_cache);
                } else {
                    if (activityadd_power) {
                        ...
                        $.extend(true, (function () {
                            ...
                            submitHandler: function (form) {
                                ...
                                if (formtype === 'activityadd') {
                                    ...
                                    if (ftype === 0) {
                                        setdata['goodsTypeId'] = $admin_goodsTypeId.attr('data-id');
                                    } else if (ftype === 2) {
                ...
                for (var m in list) {
                    switch (m) {
                        ...
                        case 'title':
                            ...
                            break;
                        case 'forwardType':
                            activityDataByType(parseInt(list[m], 10));
                            break;
                        case 'status':
        ...
        function searchActivityPosId() {
            ...
            $.ajax(apconfig)
                .done(function (resp) {
                    ...
                    if (code !== 0) {
                        console.log(resp.message);
                        dia.content('<span class="g-c-bs-warning g-btips-warn">' + (resp.message || "查询广告位置失败") + '</span>').show();
                        if (code === 999) {
                            public_tool.loginTips(function () {
                                public_tool.clear();
                                public_tool.clearCacheData();
        ...
        /*根据状态初始化编辑界面*/
        function activityDataByType(type) {
            if (!type) {
                return false;
            }
        ...
        /*根据位置初始化编辑界面*/
        function activityDataByPos(config) {
            if (!config) {
                return false;
            }
            ...
            if (pos === 1) {
                ...
                $admin_urlImage.attr({
                    'data-image': url
                }).html('<img src="' + url + "?imageView2/1/w/340/h/480" + '" alt="图像">');
            } else if (pos === 2) {
                /*右上*/
                $admin_urlImage.attr({
                    'data-image': url
                }).html('<img src="' + url + "?imageView2/1/w/644/h/240" + '" alt="图像">');
            } else if (pos === 3 || pos === 4) {
                /*右中，右下*/
                $admin_urlImage.attr({
                    'data-image': url
                }).html('<img src="' + url + "?imageView2/1/w/306/h/208" + '" alt="图像">');
            } else if (pos === 0) {
                /*顶部*/
                $admin_urlImage.attr({
                    'data-image': url
                }).html('<img src="' + url + "?imageView2/1/w/1080/h/500" + '" alt="图像">');
            } else {
                /*默认*/
                $admin_urlImage.attr({
                    'data-image': url
                }).html('<img src="' + url + "?imageView2/1/w/160/h/160" + '" alt="图像">');
            }
        }
