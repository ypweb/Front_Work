...
(function () {
    ...
    function testService() {
        var Random = Mock.Random,
            ...
            reg_sequence = /([1-9]{1}[0-9]{8,15}){1}//*id序列值*/,
        ...
        /*对外Api*/
        this.getMap = _generateMap_/*生成集合*/;
        this.getResult = _generateResult_/*生成结果集*/;
        this.getLimit = _generateLimit_/*生成范围*/;
        this.getRule = _generateRule_/*生成正则值*/;
        this.test = test/*普通测试接口*/;
        this.testToken = testToken/*生成凭证*/;
        this.testMenu = testMenu/*生成菜单权限*/;
        this.testSuccess = testSuccess/*测试成功*/;


        /*测试接口--普通*/
        function test(config) {
            return _generateResult_(_generateMap_(config), config);
        }

        /*测试接口--生成凭证*/
        function testToken(type) {
            var res,
                token = Mock.mock({
                    "id": reg_id,
        ...
        /*通用方法--生成范围*/
        function _generateLimit_(config) {
            var limit;
            /*配置信息*/
            if (config) {
        ...
        function _generateMap_(config) {
            ...
            if (typeof maptype !== 'undefined') {
                if (maptype === 'array') {
                    result[_generateLimit_(config)] = [map_obj];
                } else if (maptype === 'object') {
                    result[_generateLimit_(config)] = map_obj;
                }
            } else {
        ...
        /*通用方法--生成结果集*/
        function _generateResult_(datalist, config) {
            var result = {};
            if (config) {
